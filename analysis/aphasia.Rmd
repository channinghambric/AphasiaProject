...
#Exploratory - comparing to mean
#models more differentiated since some p's fit much better for some models
mean_bic = bic %>%
 # mutate(deltaBIC  = randomBIC - optimalBIC) %>%
  group_by(Group,everything) %>%
  summarise(meanBIC = mean(optimalBIC)) %>%
  arrange(meanBIC) %>%
  slice(1:5) %>% 
  ungroup() 
kable(mean_bic)

#both median and mean BICs return same best-fit models for each group
...
subject_median_bic = bic %>% 
  ...
  ungroup() %>%
  separate(everything, 
           into = c("model_type", "method", "param1", "param2", "param3"), #separating out model parameters
           sep = "_", 
           fill = "right")
