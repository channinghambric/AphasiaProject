...
battery_long <- pwa_dx %>%
  ...
  select(-TrueSem, -TrueMix, -TrueFor) %>%
  pivot_longer( # put in long format for outlier drops
    cols = c(TCorrPNT, TrueNonW, semantic_output, phon_output, prt, syntot., camcac., nonwdrep..),
    names_to = "test",
    values_to = "value"
  ) %>%
...
fluency_battery_lm = lm(data = lexical_battery_wide, fluency_score ~ TCorrPNT+semantic_output+phon_output+prt+syntot.+camcac.+nonwdrep..+avg_length)
