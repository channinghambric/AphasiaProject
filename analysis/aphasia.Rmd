...
battery <- pwa_dx %>%
  ...
  select(Subject, TCorrPNT, TrueSem, TrueFor, prt, syntot., camcac., catprl1,nonwdrep..,rhyprl1) %>%
  mutate(
...
#Semantic category probe test (catprl1)
...
#Rhyme Probe Test (rhyprl1)
#Test of phonological short-term memory in which participants listen to a list of three or more words and must determine whether the final word rhymes with any of the preceding words by
#saying or pointing to “Yes” or “No”. The list of words gradually increases and performance is measured as the maximum list length with 75% or 
#higher accuracy: M=2.80, SD=1.69,Range=.50–7.31.
...
fluency_battery_lm = lm(data = lexical_battery, fluency_score ~ TCorrPNT+TrueSem+TrueFor+prt+syntot.+camcac.+catprl1+nonwdrep..+rhyprl1+avg_length)
...
lexical_battery_results <- lexical_battery  %>%
  ...
  nest() %>%                                         # Nest data by cue
 mutate(
    model = map(data, ~ lm(value ~ TCorrPNT+TrueSem+TrueFor+prt+syntot.+camcac.+catprl1+nonwdrep..+rhyprl1+avg_length, data = .)) # Fit models
  )
#View results
