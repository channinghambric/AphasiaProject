...
best_control_switches = all_switches %>% filter(Group=="Control") %>% filter(full_method==control_best_method)%>%
    mutate(Group="Control")
best_pwa_switches = all_switches %>% filter(Group=="PWA") %>%filter(full_method==pwa_best_method) %>%
    mutate(Group="PWA")

comb_best_switches = rbind(best_control_switches, best_pwa_switches)

comb_best_switches$Switch_Value[comb_best_switches$Switch_Value == 2] <- 1
...
switches_recoded <- comb_best_switches %>%
  group_by(Subject) %>%
  mutate(
    # Create Cluster_ID by tracking each transition of Switch_Value 1 or 0 to 1 for each subject
    Cluster_ID = cumsum(Switch_Value == 1)
  ) %>%
  mutate(
    # Create the Switch column: mark only the first row of each Cluster_ID as a transition
    Switch = case_when(
      row_number() == 1 ~ "Switch_Transition",  # Mark the first row of each Cluster_ID
      lag(Cluster_ID) != Cluster_ID ~ "Switch_Transition",  # If Cluster_ID changes, mark as transition
      TRUE ~ NA_character_  # All other rows within a cluster are not transitions
    ),
    # Add a random ID for each unique Cluster_ID within each Subject
    Cluster_Random_ID = sapply(Cluster_ID, function(x) UUIDgenerate())
  ) %>%
  mutate(item_no = row_number())%>%
  ungroup() %>%
  left_join(all_lexical %>% group_by(Subject) %>% mutate(item_no = row_number()))


#Does avg within cluster similarity differ by Group  - essentially treating each cluster as an id variable
group_comp<-switches_recoded %>%select(Cluster_Random_ID,Group,Semantic_Similarity,Phonological_Similarity,Frequency_Value)

group_sem_model<-lmer(data=group_comp, Semantic_Similarity~Group +(1|Cluster_Random_ID))
summary(group_sem_model)
car::Anova(group_sem_model)
#sig lower for PWA

group_phon_model<-lmer(data=group_comp, Phonological_Similarity~Group +(1|Cluster_Random_ID))
summary(group_phon_model)
car::Anova(group_phon_model)
#sig lower for PWA

group_freq_model<-lmer(data=group_comp, Frequency_Value~Group +(1|Cluster_Random_ID))
...
switches_comp <- switches_recoded %>%
  group_by(Subject, Group) %>%
  slice(-1) %>%  # Removes the first row from each group
