...
battery_long <- pwa_dx %>%
  ...
  select(Subject, PNT, Semantic_Errors_PNT, Formal_Errors_PNT, Mixed_Errors_PNT, Nonword_Errors_PNT,Nonword_Repetition_Test, PRT, Synonymy_Triplet_Test, Camels_and_Cactus_Test,Immediate_Serial_Recall_Wordspan,Category_Probe_Max_Span,Rhyme_Probe_Max_Span) %>%
  mutate(
  ...
  pivot_longer( # put in long format for outlier drops
    cols = c(PNT, semantic_output,phon_output, Nonword_Repetition_Test, PRT, Synonymy_Triplet_Test, Camels_and_Cactus_Test,Immediate_Serial_Recall_Wordspan,,Category_Probe_Max_Span,Rhyme_Probe_Max_Span),
    names_to = "test",
    values_to = "value"
  ) %>%
...
# Lexical wide (for fluency analysis)
...
#long (for nested lexical analyses)
...
#do battery measures predict fluency score
fluency_battery_lm = lm(data = lexical_battery_wide, fluency_score ~ PNT+semantic_output+phon_output+PRT+Synonymy_Triplet_Test+Camels_and_Cactus_Test+Nonword_Repetition_Test+avg_length)
summary(fluency_battery_lm)

#including max probe lengths
fluency_battery_lm_add_probe = lm(data = lexical_battery_wide, fluency_score ~ PNT+semantic_output+phon_output+PRT+Synonymy_Triplet_Test+Camels_and_Cactus_Test+Nonword_Repetition_Test+avg_length+Category_Probe_Max_Span + Rhyme_Probe_Max_Span)
summary(fluency_battery_lm_add_probe)

#just max probe lengths as preds
fluency_battery_lm_just_probe = lm(data = lexical_battery_wide, fluency_score ~ Category_Probe_Max_Span + Rhyme_Probe_Max_Span)
summary(fluency_battery_lm_just_probe)
...
lexical_battery_results <- lexical_battery_long  %>% 
  ...
  nest() %>%                                         # Nest data by cue
 mutate(
    model = map(data, ~ lm(value ~ PNT+semantic_output+phon_output+PRT+Synonymy_Triplet_Test+Camels_and_Cactus_Test+Nonword_Repetition_Test+avg_length+fluency_score, data = .)) # Fit models
  )
#View results
...
#Same as above but adding probes as preds
lexical_battery_results_add_probe <- lexical_battery_long  %>% 
  group_by(cue) %>%                                   # Group by cue
  nest() %>%                                         # Nest data by cue
 mutate(
    model = map(data, ~ lm(value ~ PNT+semantic_output+phon_output+PRT+Synonymy_Triplet_Test+Camels_and_Cactus_Test+Nonword_Repetition_Test+avg_length+Category_Probe_Max_Span + Rhyme_Probe_Max_Span+fluency_score, data = .)) # Fit models
  )
#View results
model_battery_summaries_add_probe <- lexical_battery_results_add_probe %>%
  mutate(model_summary = map(model, broom::tidy)) %>%
  unnest(model_summary)
View(model_battery_summaries_add_probe)


# Just probes as preds
lexical_battery_results_just_probe <- lexical_battery_long  %>% 
  group_by(cue) %>%                                   # Group by cue
  nest() %>%                                         # Nest data by cue
 mutate(
    model = map(data, ~ lm(value ~avg_length+Category_Probe_Max_Span + Rhyme_Probe_Max_Span+fluency_score, data = .)) # Fit models
  )
#View results
model_battery_summaries_just_probe <- lexical_battery_results_just_probe %>%
  mutate(model_summary = map(model, broom::tidy)) %>%
  unnest(model_summary)
View(model_battery_summaries_just_probe)
...
emm_results <- list(
  PNT = emmeans(
    fluency_battery_lm, ~ PNT, cov.reduce = range
  ),
