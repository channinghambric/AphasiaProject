...
```{r}
battery<-pwa_dx %>% rename("Subject"="Fluency.ID") %>%select(Subject,TCorrPNT,TrueSem,TrueFor,TrueMix,prt,syntot.,camcac.,catprl1)%>%mutate(Subject=as.factor(Subject))
pwa_avg_length<-pwa_lengths %>%  group_by(Subject) %>%
  summarize(avg_length = mean(Phoneme.Count, na.rm = TRUE))
                       
#filter out controls and join battery data
...
lexical_battery<-all_indv %>% select(Subject, Group, Semantic_Similarity_mean, Phonological_Similarity_mean, Frequency_Value_mean, 
         fluency_score) %>% na.omit() %>%filter(Group=="PWA") %>%left_join(battery,by="Subject") %>%left_join(pwa_avg_length,by="Subject")
