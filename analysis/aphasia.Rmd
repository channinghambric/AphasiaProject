...
#median normalized BIC (BIC/N)
median_norm_bic = bic %>% #filter(!Subject %in% c(90009, 90019, 20050, 90030, 90033, 90028, 90026-2))%>% #trying excluding weird subs
  #mutate(deltaBIC  = randomBIC - optimalBIC) %>%
  group_by(Group,everything) %>%
  summarise(median_norm_BIC = median(optimalBIC/N)) %>%
  arrange(median_norm_BIC) %>%
  slice(1:3) %>% 
  ungroup() 
kable(median_norm_bic)

#median delta BIC
median_delta_bic = bic %>%#filter(!Subject %in% c(90009, 90019, 20050, 90030, 90033, 90028, 90026-2))%>%
 mutate(deltaBIC  = randomBIC - optimalBIC) %>%
  group_by(Group,everything) %>%
...
# median norm
median_norm_rank <-bic %>%
  #mutate(deltaBIC  = randomBIC - optimalBIC) %>%
  group_by(Subject, Group, everything) %>%
  summarise(median_norm_BIC = median(optimalBIC/N)) %>%
  arrange(median_norm_BIC, .by_group = TRUE) %>%
  ...
  separate(everything, 
           into = c("model", "method", "param1", "param2", "param3"), #separating out model parameters
           sep = "_", 
           fill = "right") %>%select(-method,-param1,-param2,-param3)%>%
  rename("BIC_Value"=median_norm_BIC)%>%
  mutate(method="med_norm_bic")

# median delta
median_delta_rank <-bic %>%
  mutate(deltaBIC  = randomBIC - optimalBIC) %>%
  group_by(Subject, Group, everything) %>%
  summarise(median_delta_BIC = median(deltaBIC), .groups = "drop_last") %>%
  arrange(desc(median_delta_BIC), .by_group = TRUE) %>%
  mutate(rank = row_number()) %>%
  slice_head(n = 3) %>%                 
  ungroup()%>%
  separate(everything, 
           ...
           sep = "_", 
           fill = "right") %>%select(-method,-param1,-param2,-param3)%>%
  rename("BIC_Value"=median_delta_BIC)%>%
  mutate(method="med_delta_bic")

# Combine to compare
comp_bic_methods<-rbind(median_norm_rank,median_delta_rank)
  

# Wide
# Checking consistency of models across BIC methods
comp_bic_wide_model <- comp_bic_methods %>%
  select(Subject, Group, method, model, rank) %>%
  pivot_wider(
    id_cols = c(Subject, Group),
    names_from = c(method, rank),
    values_from = model,
    names_glue = "{method}_rank_{rank}") %>%
    mutate(match_rank1 = if_else(med_norm_bic_rank_1 == med_delta_bic_rank_1, TRUE, FALSE),
           match_rank2 = if_else(med_norm_bic_rank_2 == med_delta_bic_rank_2, TRUE, FALSE),
           match_rank3 = if_else(med_norm_bic_rank_3 == med_delta_bic_rank_3, TRUE, FALSE))

#all ranks consistent across bic methods for all participants

# Checking diff between rank 1 and 2 BIC
comp_bic_wide_diff<- comp_bic_methods %>%
  select(Subject, Group, method, rank, BIC_Value) %>%
  pivot_wider(
    id_cols   = c(Subject, Group, method),
    names_from = rank,
    values_from = BIC_Value,
    names_prefix = "rank_")%>%
  mutate(rank1_2_diff=round(rank_2-rank_1,2))

# larger diff b/w rank 1 and 2 in a handful of indv
#	90009, 90019, 20050 (the 1 PWA), 90030, 90033, 90028, 90026-2
# 20050 - Rank1 = pstatic, Rank2 = pglobal



# Count how many times each model appears in ranks 1–3, per method & Group
rank_counts <- comp_bic_methods %>%
  filter(rank %in% 1:3) %>%
  group_by(Group, method, model, rank) %>%
  summarise(n = n(), .groups = "drop")

# Plot
ggplot(rank_counts, aes(x = model, y = n, fill = factor(rank))) +
  geom_col(position = position_dodge(width = 0.8), width = 0.7, color = "black") +
  facet_wrap(~Group, scales = "free_y") +
  scale_fill_brewer(palette = "Blues", name = "Rank") +
  theme_few() +
  theme(
  ...
  labs(
    ...
    title = "Frequency of Models Ranked 1–3 by Group")+
  geom_text(
    aes(label = n),
...
best_models <- all_models %>%
  filter(paste(Group,everything) %in% paste(mean_bic$Group, mean_bic$everything)) %>%
  group_by(Group)
