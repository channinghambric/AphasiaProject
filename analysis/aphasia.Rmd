...
fluency_battery_lm = lm(data = lexical_battery, fluency_score ~ TCorrPNT+TrueSem+TrueFor+prt+syntot.+camcac.+catprl1)
...
lexical_battery_results <- lexical_battery  %>%
  ...
  nest() %>%                                         # Nest data by cue
 mutate(
    model = map(data, ~ lm(value ~ TCorrPNT+TrueSem+TrueFor+prt+syntot.+camcac.+catprl1, data = .)) # Fit models
  )
#View results
...
plot_pnt <- emmip(emm_results$TCorrPNT, ~ TCorrPNT, CIs = TRUE) +
  labs(title = "PNT by Fluency Score",
       x = "TCorrPNT", 
...
plot_TrueFor <- emmip(emm_results$TrueFor, ~ TrueFor, CIs = TRUE) +
  labs(title = "Form Errors by Fluency Score",
       x = "TrueFor", 
       y = "Estimated Fluency Score")  +
  theme_few() +
  theme(aspect.ratio = 1.5, legend.position = "top")
...
plot_syntot <- emmip(emm_results$syntot., ~ syntot., CIs = TRUE) +
  labs(title = "Synonymy Judgments by Fluency Score",
       x = "syntot", 
       y = "Estimated Fluency Score")  +
  theme_few() +
  theme(aspect.ratio = 1.5, legend.position = "top")
...
#But those with higher frequency scores also produced more form errors
```
### Lexical plots

```{r}

#just significant predictors
...
#for verbal semantic processing tasks (syntot), high performance assc with lower inter-item phonological similarity (less interference/more control over semantic processing)
#for NON verbal (camcac), high score assc with high avg inyter-item phonological similarity
```
## Cluster Analyses
```{r}
