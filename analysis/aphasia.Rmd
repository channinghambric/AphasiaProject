...
#Using Best Method, Explore Within and Across Cluster Similarity
```{r}

```


#Exploring aphasia subtypes
```{r}
#Looking at distribution of best models across subtypes
pwa_fluency_linked<-pwa_fluency_cutoff %>% mutate(Subject=ID) %>% select(Subject, LangDx) %>% unique()
subject_best_models%>% filter(Group=="PWA") %>% left_join(pwa_fluency_linked,by="Subject") %>%
  mutate(LangDx = str_to_lower(LangDx)) %>%mutate_all(~str_replace_all(., "\\s+", "")) %>%
    group_by(Group, LangDx, model_type) %>%
    count() %>%
...
#Exploring Lexical differences
#Probably need to integrate those anomic subtypes

#semantic
sem_sim_comparison %>%
  filter(Group == "PWA") %>%
  left_join(pwa_fluency_linked, by = "Subject") %>%
  mutate(LangDx = str_to_lower(LangDx)) %>%
  mutate(LangDx = str_replace_all(LangDx, "\\s+", "")) %>%
  filter(!is.na(LangDx) & !is.na(avg_sem_sim)) %>%
  mutate(LangDx = factor(LangDx)) %>%  
  ggplot(aes(x = LangDx, y = avg_sem_sim, color = LangDx)) +
  geom_beeswarm(alpha = 0.6, cex = 2,size=3) +  
  labs(y = 'Semantic Similarity', x = '') +
  theme_few() +
...
phon_sim_comparison %>%
  ...
  mutate(LangDx = str_to_lower(LangDx)) %>%
  mutate(LangDx = str_replace_all(LangDx, "\\s+", "")) %>%
  filter(!is.na(LangDx) & !is.na(avg_phon_sim)) %>%
  mutate(LangDx = factor(LangDx)) %>%  
  ggplot(aes(x = LangDx, y = avg_phon_sim, color = LangDx)) +
  geom_beeswarm(alpha = 0.6, cex = 2, size=3) +  
  labs(y = 'Phonological Similarity', x = '') +
  theme_few() +
  theme(legend.position = "none") +
...
freq_comparison %>%
  ...
  mutate(LangDx = str_to_lower(LangDx)) %>%
  mutate(LangDx = str_replace_all(LangDx, "\\s+", "")) %>%
  filter(!is.na(LangDx) & !is.na(avg_freq)) %>%
  mutate(LangDx = factor(LangDx)) %>%  
  ggplot(aes(x = LangDx, y = avg_freq, color = LangDx)) +
  geom_beeswarm(alpha = 0.6, cex = 2, size=3) +  
  labs(y = 'Word Frequency', x = '') +
  theme_few() +
  theme(legend.position = "none") +
