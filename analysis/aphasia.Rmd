...
lowest_bic %>%
  ...
  geom_density(fill = "#69b3a2", color = "#e9ecef", alpha = 0.8)+xlim(0,400)

#BY GROUP BEST MODELS
#median delta BIC
#a lot of the models have highly similar values due to most p's having relatively small delta bics across models
...
#both median and mean BICs return same bets-fit models for each group
...
# for table
table_df<-lexical_battery%>%left_join(demos)%>%select(-cue,-value)%>%unique()
```
